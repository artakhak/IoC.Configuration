...
namespace IoC.Configuration.DiContainerBuilder.FileBased
{
    public class FileBasedConfiguration : DiContainerBuilderConfiguration
    {
                                       ...
                                       IList<ModuleInfo> allGeneratedModuleInfos)
        {
            ...
            {
                if (!pluginSetup.Enabled || !pluginSetup.PluginImplementationElement.Enabled)
                    continue;

                serviceImplementationElements.Add(pluginSetup.PluginImplementationElement);
                dynamicAssemblyBuilder.AddReferencedAssembly(pluginSetup.PluginImplementationElement.ImplementationType);
                                             ...
                                             [NotNull] [ItemNotNull] IEnumerable<BindingImplementationConfigurationForFile> bindingImplementationConfigurations)
        {
            foreach (var bindingImplementationConfiguration in bindingImplementationConfigurations)
            {
                ...
                if (bindingImplementationConfiguration.Parameters != null)
                    foreach (var parameter in bindingImplementationConfiguration.Parameters)
                        dynamicAssemblyBuilder.AddReferencedAssembly(parameter.ValueType);

                if (bindingImplementationConfiguration.InjectedProperties != null)
                    foreach (var injectedProperty in bindingImplementationConfiguration.InjectedProperties)
                        dynamicAssemblyBuilder.AddReferencedAssembly(injectedProperty.ValueType);
            }
        }
        ...
        private void AddSettingsRequestorBindings([NotNull] IList<BindingConfigurationForFile> servicesToAddBindingsTo, [NotNull] IDynamicAssemblyBuilder dynamicAssemblyBuilder)
        {
            ...
            dynamicAssemblyBuilder.AddReferencedAssembly(Configuration.SettingsRequestor.ImplementationType);
        ...
        private void AddStartupActionBindings([NotNull] IList<BindingConfigurationForFile> servicesToAddBindingsTo, [NotNull] IDynamicAssemblyBuilder dynamicAssemblyBuilder)
        {
            ...
            foreach (var startupAction in Configuration.StartupActions.StartupActions)
            {
                ...
                dynamicAssemblyBuilder.AddReferencedAssembly(startupAction.ImplementationType);
            }
        ...
        protected override IEnumerable<object> GenerateAllNativeModules()
        {
            ...
            using (var dynamicAssemblyBuilder = GlobalsCoreAmbientContext.Context.StartDynamicAssemblyBuilder(dynamicallyGeneratedAssemblyFilePath, OnDynamicAssemblyBuildCompleted, true))
            {
        ...
        private void Load([NotNull] IConfigurationFileContentsProvider configurationFileContentsProvider)
        {
            ...
            catch (Exception e)
            {
                ...
                throw new Exception($"Failed to parse the xml file '{configurationFileContentsProvider.ConfigurationFileSourceDetails}'.");
            }
        }
                                                       ...
                                                       [NotNull] IList<ModuleInfo> allGeneratedModuleInfos)
        {
            ...
            {
                dynamicAssemblyBuilder.AddReferencedAssembly(service.ServiceType);
                services.Add(BindingConfigurationForFile.CreateBindingConfigurationForFile(service.ServiceType, service.RegisterIfNotRegistered, service.Implementations));
            }
        ...
        private void ValidateRequiredSettings([NotNull] ISettings settings, [NotNull] ISettingsRequestor settingsRequestor)
        {
            foreach (var requierdSetting in settingsRequestor.RequiredSettings)
            {
                ...
                if (setting == null || setting.DeserializedValue == null ||
                    !requierdSetting.ValueDataType.IsAssignableFrom(setting.DeserializedValue.GetType()))
                {
                    ...
                    if (setting == null)
                        ...
                        errorMessage.Append($"Required setting '{requierdSetting.Name}' should be of type '{requierdSetting.ValueDataType.FullName}'. Actual type is '{setting.ValueType.FullName}'. The setting is in ");
        ...
        private class ConcreteClassesRegistrationsModule : ModuleAbstr
        {
            ...
            [NotNull]
            [ItemNotNull]
            private readonly HashSet<Type> _registeredClasses = new HashSet<Type>();
            ...
            public ConcreteClassesRegistrationsModule([NotNull] IConfiguration configuration, [CanBeNull] [ItemNotNull] IEnumerable<IDiModule> additionalDiModules)
            {
                ...
                AccountForRegistrationsInDependencyInjectionElements(_configuration.PluginsSetup.AllPluginSetups.Select(x => x.DependencyInjection));
            }
            ...
            private void AccountForRegistrationsInDependencyInjectionElements(IEnumerable<IDependencyInjection> dependencyInjectionElements)
            {
                ...
                {
                    foreach (var moduleElement in dependencyInjection.Modules.Modules)
                        if (moduleElement.DiModule is IDiModule)
                            AccountForRegistrationsInModule((IDiModule) moduleElement.DiModule);

                    foreach (var service in dependencyInjection.Services.AllServices)
                        if (service.ServiceType.IsClass && !_registeredClasses.Contains(service.ServiceType))
                            _registeredClasses.Add(service.ServiceType);
                }
            }
            ...
            private void AccountForRegistrationsInModule([NotNull] IDiModule diModule)
            {
                foreach (var serviceBindingConfiguration in diModule.ServiceBindingConfigurations)
                    if (serviceBindingConfiguration.ServiceType.IsClass && !_registeredClasses.Contains(serviceBindingConfiguration.ServiceType))
                        _registeredClasses.Add(serviceBindingConfiguration.ServiceType);
            }
            ...
            private void ProcessConfigurationFileElement([NotNull] IConfigurationFileElement configurationFileElement, ref bool stopProcessing)
            {
                ...
                else if (configurationFileElement is INamedValue)
                    RegisterSelfBoundService(((INamedValue) configurationFileElement).ValueType);
            }
