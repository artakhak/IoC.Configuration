...
if not exist "%TestFiles%\OROptimizer.Shared\" mkdir "%TestFiles%\OROptimizer.Shared"
...
xcopy "%NugetPackagesDir%\oroptimizer.shared\2.0.0\lib\netstandard2.0\OROptimizer.Shared.*" %ThirdPartyLibsDir% /Y

REM If the next line is enabled, make sure to delete all OROptimizer.Shared.* files from %ThirdPartyLibsDir% folder
REM xcopy "K:\Projects\OROptimizer\MyGitHubProjects\OROptimizer.Shared\OROptimizer.Shared\bin\Debug\netstandard2.0\OROptimizer.Shared.*" %ThirdPartyLibsDir% /Y

REM Copy IoC extensions
REM xcopy "%NugetPackagesDir%\ioc.configuration.autofac\1.0.2\lib\netstandard2.0\IoC.Configuration.Autofac.*"  %DynamicDllsDir%\ContainerImplementations\Autofac /Y
REM xcopy "%NugetPackagesDir%\ioc.configuration.ninject\1.0.2\lib\netstandard2.0\IoC.Configuration.Ninject.*"  %DynamicDllsDir%\ContainerImplementations\Ninject /Y

xcopy "K:\Projects\OROptimizer\MyGitHubProjects\IoC.Configuration\IoC.Configuration.Autofac\bin\Debug\IoC.Configuration.Autofac.*"  %DynamicDllsDir%\ContainerImplementations\Autofac /Y
xcopy "K:\Projects\OROptimizer\MyGitHubProjects\IoC.Configuration\IoC.Configuration.Ninject\bin\Debug\IoC.Configuration.Ninject.*"  %DynamicDllsDir%\ContainerImplementations\Ninject /Y
...
xcopy %SolutionDir%TestProjects.Plugin1WebApiControllers\bin\%ConfigurationName%\TestProjects.Plugin1WebApiControllers.*  %DynamicDllsDir%\PluginDlls\Plugin1 /Y
