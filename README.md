...
The main functions of the IoC.Configuration library are:
1)  Container agnostic configuration of dependency injection using an XML configuration file. The file has a section where a container can be specified that will be handling dependency injection resolutions.
...
## Configuration
Dependency injection can be configured either using a file-based or code-based configuration builder.

Bindings can be specified in three different ways:

-In the configuration file (for file-based configuration only).
...
In file-based configuration, the modules (Autofac, Ninject, or implementations of IoC.Configuration.IDiModule) can be specified in a configuration file in element iocConfiguration/dependencyInjection/modules/module
...
**Below is an example of specifying bindings in AddServiceRegistrations() method in a subclass of IoC.Configuration.DiContainer.ModuleAbstr (note the example is taken from the test project, so the class names are not user-friendly).**
